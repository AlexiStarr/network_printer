...
static void* handle_client(void* arg) {
    ...
    while (running) {
        ...
        if (req != NULL) {
            ...
            if (resp != NULL) {
                send(client_sock, resp->data, strlen(resp->data), 0);
                free_response(resp);
            }
            free_request(req);
        }
...
int start_driver_server(int port) {
    ...
    if (server_sock < 0) {
        fprintf(stderr, "[Driver] 错误：无法创建套接字\n");
    ...
    if (bind(server_sock, (struct sockaddr*)&server_addr, sizeof(server_addr)) < 0) {
        fprintf(stderr, "[Driver] 错误：端口 %d 绑定失败（可能已被其他程序占用）\n", port);
        close(server_sock);
        printer_free(global_printer);
        return -1;
    }
